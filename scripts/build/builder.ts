...
import { build, type BuildOptions, type BuildContext, type BuildResult } from "esbuild";
...
export class Builder {
  ...
  private createBuildConfig(overrideConfig: Partial<BuildOptions> = {}): BuildOptions {
    const { outfile, entryPoint, production } = this.config.builder;

    console.log(chalk.cyan(`\n⚙️  Configuring build...`));
    console.log(chalk.dim(`   Mode: ${production ? "production" : "development"}`));
  ...
  async build(overrideConfig: Partial<BuildOptions> = {}): Promise<BuildResultConfig> {
