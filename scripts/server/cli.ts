...
export class CLI {
  ...
  initialize(): void {
    ...
    console.log(chalk.dim(`  • ${chalk.bold("Q")} or ${chalk.bold("Ctrl+C")} `).padEnd(25) + chalk.gray("Exit Fastcord"));

    if (this.config.adb.enabled) {
      if (this.adbManager.isAvailableAndInstalled()) {
        const packageName = this.adbManager.getPackageName();

        console.log(chalk.dim(`  • ${chalk.bold("R")}`).padEnd(25) + chalk.gray(` Restart Discord (${chalk.blue(packageName)})`));
        console.log(chalk.dim(`  • ${chalk.bold("S")}`).padEnd(25) + chalk.gray(` Force stop Discord (${chalk.blue(packageName)})`));
