...
export function getConfig(): Config {
  ...
  program
    .option("-p, --port <number>", "Server port", String(defaultServerConfig.port))
    .option("-h, --host <string>", "Server host", defaultServerConfig.host)
    .option("--bundle-path <string>", "Path to the bundle", defaultServerConfig.bundlePath);

  program
    .option("-e, --entry-point <string>", "Build entry point", defaultBuilderConfig.entryPoint)
    .option("-o, --outfile <string>", "Output file", defaultBuilderConfig.outfile)
    .option("--production", "Production mode", defaultBuilderConfig.production)
    .option("-w, --watch", "Watch for file changes", defaultBuilderConfig.watch);

  program
    .option("--adb", "Enable ADB integration", defaultAdbConfig.enabled)
    .option("--package-name <string>", "Discord package name", defaultAdbConfig.packageName);
