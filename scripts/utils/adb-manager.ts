...
export function isAppInstalled(packageName: string): boolean {
  try {
    const output = execSync(`adb shell pm list packages ${packageName}`).toString();
...
export function startApp(packageName: string): void {
  try {
    ...
    execSync(`adb shell monkey -p ${packageName} -c android.intent.category.LAUNCHER 1`, { stdio: "ignore" });
  } catch (error) {
